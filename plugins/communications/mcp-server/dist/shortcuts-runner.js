...
const DELIMITER = '|||MBG|||';
export async function groupShortcutInstalled() {
    try {
 ...
 */
export async function sendGroupViaShortcut(recipients, text) {
    ...
    try {
        ...
        const e = err;
        throw new Error(`Shortcut "${GROUP_SHORTCUT_NAME}" failed: ${e.stderr?.trim() || e.message || 'unknown error'}`);
    }
    finally {
