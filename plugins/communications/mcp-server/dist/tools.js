...
export function registerTools(server, chatDb) {
    server.tool('imessage_send', 'Send an iMessage. Provide EITHER recipient (phone or email) OR chatGuid (existing chat). Not both.', {
        recipient: z
            .string()
            .optional()
    ...
    server.tool('imessage_find_group', 'Find existing chats containing ALL the given participants (2+ phone numbers/emails). Returns exact matches (participant set is precisely these people) first, then supersets (these people plus others). Use BEFORE any multi-recipient send — an existing thread beats creating anything.', {
        participants: z
    ...
    server.tool('imessage_send_group', 'Send one message to multiple people as a GROUP thread. Order of attempts: (1) existing thread with exactly these participants via AppleScript; (2) if none, create the group via the "MBG Group Send" shortcut (requires one-time install by comm-imessage-fast-setup); (3) if that fails and fallbackToIndividual is true, send to each person individually. ALWAYS confirm mode (group vs individual) and failover with the user before calling.', {
        participants: z
            .array(z.string().min(3))
            .min(2)
            .max(30)
        ...
        if (await shortcuts.groupShortcutInstalled()) {
            try {
                ...
                if (!fallbackToIndividual) {
                    throw new Error(`${err.message} — no fallback was authorized. Options: retry, send individually (fallbackToIndividual), or have the user start the group manually in Messages once.`);
                }
                // fall through to 3
            }
        }
        else if (!fallbackToIndividual) {
            throw new Error(`No existing group thread with these participants, and the "${shortcuts.GROUP_SHORTCUT_NAME}" shortcut is not installed. ` +
    ...
    server.tool('imessage_send_individual', 'Send the SAME message to multiple people as separate 1:1 conversations (not a group thread). Use when the user explicitly chooses individual sends over a group. Reports per-recipient success/failure.', {
        recipients: z
            .array(z.string().min(3))
            .min(2)
            .max(50)
    ...
    server.tool('imessage_search_chats', 'Search chat.db for chats matching a query against display name, chat GUID, or participant handle (phone/email). Returns chat GUIDs + participants.', {
    ...
    server.tool('imessage_read_chat', 'Read the most recent messages in a chat by GUID. Includes both sent and received messages.', {
    ...
    server.tool('imessage_poll_new', 'Return inbound messages newer than the persisted marker, then advance the marker. First-ever call initializes the marker to the current chat.db max rowid and returns nothing.', {
        ...
        if (maxSeenRowId > marker)
            setMarker(maxSeenRowId);
        return {
            content: [
                {
                    type: 'text',
    ...
    server.tool('imessage_peek_new', 'Same query as imessage_poll_new but does NOT advance the marker. Safe to call repeatedly for inspection.', {
        limit: z.number().int().min(1).max(500).default(100),
    }, async ({ limit }) => {
        let marker = getMarker();
        if (marker === null)
        ...
        return {
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({ marker, count: messages.length, messages }, null, 2),
                },
            ],
        };
    });
    server.tool('imessage_reset_marker', 'Set the poll marker to a specific rowid or to the current chat.db max rowid.', {
        rowId: z
            .number()
            .int()
            .min(0)
